...
export function Sidebar({ displayed, toggleMenu }) {
    ...
    const about = [
        ...
        'Para explicar um pouco mais sobre a idéia dessa aplicação irei conceituá-los um pouco sobre comportamentos arquetípicos e como comecei a criação do projeto.',
        'Estudos das áreas de psicologia e neurociência relatam que aproximadamente até os 5 anos de idade nós, seres humanos, não temos noção de individualidade, nos enxergamos como parte de um todo. Logo após o termino dessa fase, comecei a perceber padrões dos mais diversos tipos e alguns lapsos de abstrações comportamentais começaram a se fazer presentes em minha realidade, como qualia, consciência e questionamentos sobre a motivação de meus atos e das pessoas que convivia. Durante a adolescência isso diminuiu, porém, após essa fase, passei anos estudando de maneira psiconauta. Então, percebi que respostas para esses tipos de questões (que acredito que todo mundo já se fez em algum momento da vida) talvez não sejam tão exatas quanto imaginei, mas que podem ser exploradas a partir de alguma estrutura lógica.',
        'Compreender o sincretismo que existe na história da humanidade pode nos ajudar a entender nossos comportamentos. Carl Jung conseguiu reduzir as figuras mitológicas (não de forma pejorativa) a alguns modelos arquetípicos, e você pode mesmo pode perceber como alguns deuses de diferentes mitologias se comportam de forma bem parecida. Ainda falando de comportamento, você já percebeu como alguns atores/atrizes são extremamente realistas em seus papéis de atuação? Nesse cenário, o ator incorpora características de algum(ns) arquétipo(s). As vezes você até percebe qual figura arquetípica ele está assumindo, e com o decorrer da história entende as causas de seu comportamento. O mesmo acontece em nossas vidas, embora seja difícil admitir.',
        'De acordo com o arquétipo de cada personagem, uma história individual se desenvolve. Por quê não trazer o poder os arquétipos e da atuação para sua vida e mudar a sua história? Não se trata de fantasia, mas sim de ter satisfação na história em que se vive. Trazer um pouco de história e ajudá-lo nesse caminho é o propósito do MagickHub.',
        ...
        <>
            <Aside displayed={displayed}>
                ...
                <div className={displayed ? 'container active' : 'container'}>
                    <button className="close" onClick={() => { closeMenu(); setContent(false) }} >
